...
export function useAccountingDashboardSummary() {
  ...
  const selectedCompanyId = isSuperAdmin ? companyId : company?.id;
  const enabled = DATA_SOURCE === "api" && authReady && isAuthenticated && !terminalAuthHandling && operatorReady && (!isSuperAdmin || companyReady);

  return useQuery({
    queryKey: [...queryKeys.accountingDashboardSummary(selectedCompanyId, activeBranchId), generation],
    queryFn: async () => {
      const response = await apiClient<{ data: AccountingDashboardSummary }>("/accounting/dashboard-summary", { locale, ...(companyId ? { companyId } : {}) });
