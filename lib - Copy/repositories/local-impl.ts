...
export class LocalCustomerRepository implements CustomerRepository {
  ...
  async create(customer: Customer): Promise<MutationResult<Customer>> {
    ...
    if (this.ctx.customers.some((c) => c.id === customer.id)) {
      return {
        success: false,
        error: {
          code: "DUPLICATE_ID",
          message: `المعرف ${customer.id} مستخدم بالفعل / ID ${customer.id} is already in use`,
        },
      };
    }
...
export class LocalSupplierRepository implements SupplierRepository {
  ...
  async create(supplier: Supplier): Promise<MutationResult<Supplier>> {
    if (this.ctx.suppliers.some((s) => s.id === supplier.id)) {
      return {
        success: false,
        error: { code: "DUPLICATE_ID", message: `المعرف ${supplier.id} مستخدم بالفعل` },
      };
    }
