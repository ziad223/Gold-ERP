...
| DARFUS-UIUX-UX12-FULL-REGRESSION-01 | 132 direct Chrome AR/EN Light/Dark responsive checks, zero overflow, health/DB read-only proof, 14 focused UX suites and typecheck passed; one mobile reset accessible-name repair applied; print headless runner unavailable and synchronized build exit was not captured | BLOCKED_DARFUS_UIUX_UX12_PRINT_EXPORT_AND_BUILD_EVIDENCE_INCOMPLETE |
| DARFUS-UIUX-UX12B-FINAL-EVIDENCE-CLOSEOUT-BUILD-CONSOLE-01 | Build worker exit 1 was proven from a pre-existing rollback artifact; 18-route raw Chrome logs showed 0 errors/0 warnings; DataToolbar AR/EN name and isolated rollback parity passed; official DB unchanged | BLOCKED_DARFUS_UIUX_UX12B_BUILD_AND_RAW_EVIDENCE_INCOMPLETE |
| DARFUS-UIUX-UX12B-BUILD-RAW-EVIDENCE-RECOVERY-CLOSEOUT-01 | Build scope root cause proven; no repository fix applied; 18-route Chrome console logs 0/0; DB zero-write and DataToolbar/hash proofs preserved | WAITING_OWNER_APPROVAL_FOR_PROVEN_MINIMUM_BUILD_SCOPE_REMEDIATION |
