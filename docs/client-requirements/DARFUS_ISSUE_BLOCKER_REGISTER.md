...
| DARFUS-UX10-SETTINGS-AUDIT-PRESENTATION-001 | Settings/Audit surfaces needed a consistent scoped presentation boundary for dense data, long identifiers and focus treatment | Settings/Audit UX10 | P3 | CLOSED | Scoped CSS only; setting/audit values, handlers, routes, permissions and authorities unchanged; browser/tests/typecheck/build passed. |

| DARFUS-UX11-SNAPSHOT-COMPLETENESS-001 | Initial UX11 baseline copied only selected source files, causing transient TypeScript missing-dependency errors inside the archive | UX11 evidence archive | P3 | CLOSED | Completed the archive dependency copies; `npm run typecheck` passed. No product behavior or runtime authority changed. |

| DARFUS-UX11B-DIRECT-PREVIEW-COVERAGE-001 | Required direct browser mounts for ReceiptPreview and BarcodeLabelPreview were not available in the current source/runtime | UX11B evidence closeout | P3 | BLOCKED | Source declarations were inspected; no current consumer/mount was found. UX11B forbids adding a production route or test fixture solely for evidence. |
| DARFUS-UX11B-PRINT-EXPORT-FIXTURE-001 | Local-Chrome print-export attempt reached the temporary runtime but the existing localized fixture returned HTTP 404 | UX11B evidence closeout | P3 | BLOCKED | Evidence-only runner used verified Chrome at 840x1180; root/template markers were zero, console contained only the 404 resource error, and no mutation occurred. |

| DARFUS-UX11C-STALE-PRINT-EXPORT-TEST-001 | Existing print-export test has one case with no navigation before querying its fixture marker | UX11C disposable evidence | P3 | BLOCKED | Local Chrome fixture proof passed after disposable locale alias; the exact test run was 16/17 and the failing case timed out with element not found because its body omits `page.goto`. No source/test change was made. |
| DARFUS-UX12-PRINT-HEADLESS-ENV-001 | Project print-export command cannot launch Playwright headless because the configured `headless_shell.exe` is missing | UX12 full regression | P3 | OPEN | Direct local Chrome UI was available and Print controls rendered; automated print-export evidence is incomplete. No browser installation was authorized. |
| DARFUS-UX12B-BUILD-ARTIFACT-001 | TypeScript build fails in a pre-existing UX11 rollback copy with missing `print-types` | UX12B evidence closeout | P3 | OPEN | Compilation passed; TypeScript reported the exact rollback artifact path and worker exit code 1. No cleanup or product change authorized. |
| DARFUS-UX12B-BUILD-SCOPE-RECOVERY-001 | `tsconfig.json` broad globs include ignored archive/evidence files | UX12B recovery | P3 | OWNER_GATED | Effective config includes 90 files under `backups/**`; no product import exists. Proposed narrow exclusion is documented in the Owner Decision Packet and not applied. |
