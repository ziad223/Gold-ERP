...
- F008: `/operator/current` was attempted before Branch READY and provider failure cleared the still-valid operator. `428e9dd` makes restore readiness/generation aware, exactly once per authenticated READY generation, and keeps the guard loading until authoritative. Secure hard refresh retained the operator and allowed route without verification fallback.
- F009: the login-time technical session could lack a device association, so logout orphaned its operator session. `44f9964` uses a stable technical-session fingerprint and transactional exact-owned revocation; `25c0d45` preserves the separate user-wide security revocation path. Secure logout ended with active owned technical/operator counts `0/0`.
- F010: investigation confirmed fixed-shell Branch bootstrap and module/notification lifecycles could start without effective permission. `3fabf5b` uses authenticated fixed-shell Branch metadata and permission-gates core modules, notification REST, and SSE. The deliberate safe denied API probe remains canonical `403`.
- Observability: the prior response-time token could not classify some aborted/client-disconnected paths. `3bb60c0` emits one numeric monotonic-duration terminal record with explicit completed/aborted/client-disconnected outcome; the protected app entry change is limited to installing this middleware.
- Validation: new tests passed `5/5`, all Node tests `56/56`, permission baseline `128/128`, typecheck, and lint with zero errors/18 inherited warnings. Existing employee authorization, Company/Branch, error, request-correlation, and notification/SSE verifiers passed. Production build remains deferred to preserve the active runtime.
- Decision: `AUTHORIZATION-RUNTIME-FIX-CONT3 = COMPLETE`; F004–F010 and OBSERVABILITY-F001 are resolved; open release-blocking authorization/Product regressions are zero. `RELEASE_READY = NO`; Staging and Production remain unauthorized. Exact next marker: `AUTHORIZATION-RUNTIME-ACCEPT-CONT2`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-ACCEPT-CONT1 — 2026-07-29

- Checkpoint/package: started at `ef1093b` on `main`; zero staged files, 11 stashes, no remotes, protected semantic zero, required declaration hash, reused 3000/8000/5432, and official `darfus_erp` `51/51/0`. The external encrypted package passed exact-file, ACL, hash, presence-only child, and parent-cleanup checks and remained unchanged.
- Accepted runtime: the dedicated non-admin fixture completed Branch-shell login, employee-code/PIN verification, active operator bootstrap, one explicit/default Branch, allowed dashboard rendering and a safe `200` GET. Privileged navigation was absent; a direct denied route exposed no protected data, and a safe denied GET returned canonical correlated `403` without partial data. Branch switching is `NOT_APPLICABLE` because the fixture has one assignment and a fixed Branch shell.
- `FULL-REGRESSION-F008`: hard refresh returned to employee verification although the backend operator session was active, unexpired, and authorization-version valid. Source correlation shows `/operator/current` uses the Branch-transition-guarded default client, records `CURRENT_FAILED`, and is not retried when Branch READY becomes true.
- `FULL-REGRESSION-F009`: UI logout revoked the technical session but left the active operator session because the technical session created at login had no stored device association. Fixture-scoped cleanup used supported login/end-session/logout APIs and finished with zero active technical/operator sessions. No direct database cleanup or fixture/business mutation was used.
- Validation/decision: focused contracts passed `30/30`; permission baseline `128/128`, typecheck, targeted lint, protected hash, and diff check passed. `FULL-REGRESSION-F004` through `F007` remain resolved. `AUTHORIZATION-RUNTIME-ACCEPT-CONT1 = PARTIAL`; F008 and F009 are open; `RELEASE_READY = NO`. Exact next marker: `AUTHORIZATION-RUNTIME-FIX-CONT3`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-TEST-FIXTURE-CONT1 — 2026-07-29

- Checkpoint: `34d3de6` (`docs: record employee runtime credential boundary`). Read-only preflight retained `main`, 11 stashes, no remotes, zero staged files, protected semantic zero, the required declaration hash, existing 3000/8000 listeners, and `darfus_erp` at `51/51/0` with zero idle transactions and waiting locks.
- Authorized local fixture: `QA_EMPLOYEE_AUTH_RUNTIME_001` was created through the employee authorization and system-account domain services. Creation was identity-first and immediately verified with zero implicit mappings; the sole active Branch mapping and default were then selected explicitly. The dedicated non-admin QA role resolves one safe read capability and excludes one safe administration capability. The fixture is active, has a valid PIN credential, and is not Super Admin.
- Branch shell/package: a dedicated active fixed-Branch shell was created for the explicit default Branch and normalized for non-interactive local acceptance. A current-user Windows DPAPI/SecureString package outside Git contains encrypted runtime credentials only; its loader passed a presence-only child check and the package ACL allows only the current user and SYSTEM. The package has one fixed Branch assignment, so in-session switching is not applicable; a separate unassigned active Branch candidate exists for denial evidence.
- Validation/safety: focused Node contracts passed `30/30`; permission baseline `128/128`, typecheck, targeted lint, and diff check passed. No Product, harness, migration, configuration, runtime-process, or unrelated business-data change occurred. The temporary non-secret provisioner file was removed; any empty temporary directory is non-secret environment residue.
- Decision: `AUTHORIZATION-RUNTIME-TEST-FIXTURE-CONT1 = COMPLETE`; `LOCAL_QA_EMPLOYEE_FIXTURE = READY`; `SECURE_DPAPI_PACKAGE = READY`. `RELEASE_READY = NO`; exact next marker `AUTHORIZATION-RUNTIME-ACCEPT-CONT1`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-ACCEPT-CONT1 — 2026-07-29

- Checkpoint: `4564b37` (`docs: resolve Branch readiness financial regression`). Read-only preflight retained `main`, 11 stashes, no remotes, zero staged files, zero protected semantic drift, the required `next-env.d.ts` hash, existing 3000/8000 listeners, and official `darfus_erp` at `51/51/0` with zero idle transactions and waiting locks.
- Credential gate: all three required approved employee Branch-shell process variables were absent. No wrapper, browser context, login, employee selection, PIN verification, or direct API attempt was made; no credential value was accessed or retained.
- Decision: `AUTHORIZATION-RUNTIME-ACCEPT-CONT1 = BLOCKED`; `BLOCKER = APPROVED_EMPLOYEE_CREDENTIALS_NOT_AVAILABLE`; `PRODUCT_RESULT = NOT_PROVEN`. This is not a new Product finding and does not reopen `FULL-REGRESSION-F004` through `F007`, which remain resolved by prior code and isolated/runtime acceptance as recorded.
- Safety: no Product, configuration, runtime, migration, or official database mutation occurred. `RELEASE_READY = NO`; exact next marker remains `AUTHORIZATION-RUNTIME-ACCEPT-CONT1`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-FIX-CONT2 — 2026-07-29

- Starting checkpoint: `d621460` (`docs: record employee authorization repair boundary`). The known generated `next-env.d.ts` drift was normalized byte-for-byte from `HEAD`, not staged, and retained its required hash. Existing 3000/8000 services were reused; none was started, stopped, or restarted.
- Reproduction and cause: the unchanged harness reproduced F007 as two customer-invoice reads before the observed Branch-B READY boundary; statement-v2 and credit were unaffected. The BranchContext provider had published the accessor and cleared the imperative transition guard before the READY commit.
- Repair: `80b9909` keeps the guard closed while committing READY, then publishes the accessor in a layout effect before passive query effects. It preserves Branch query-key isolation/cancellation, Company-scoped notifications, and the employee authorization repair.
- Validation: the new Branch readiness ordering test failed before the repair and passed after. Focused context/error/notification/employee contracts passed `42/42`; permission baseline `128/128`, typecheck and targeted lint passed. The unchanged reuse-only harness passed: A→B observed READY false then READY true, pre-READY financial requests `0`, no context error, controlled Branch-B absence only after READY, and notification/module/logout contracts remained non-regressed.
- Safety/decision: wrappers and credentials were removed; no generated evidence was committed. Official `darfus_erp` remains `51/51/0`, and no migration/data mutation or runtime-process control occurred. `AUTHORIZATION-RUNTIME-FIX-CONT2 = COMPLETE`; `FULL-REGRESSION-F007 = RESOLVED`. Real employee runtime acceptance remains blocked by absent approved credentials. `RELEASE_READY = NO`; exact next marker `AUTHORIZATION-RUNTIME-ACCEPT-CONT1`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-FIX-CONT1 — 2026-07-29

- Starting checkpoint: `7387ac2` (`docs: record employee authorization findings`). The only preflight repair was the authorized byte-for-byte normalization of generated `next-env.d.ts` from `HEAD`; it remains unstaged at the required hash. Existing 3000/8000 services were reused and not controlled.
- Product repair: commits `0e72f28` and `56c0538` make employee creation identity-only, remove Company/admin Branch prefill, make Branch access explicitly selected, and route completion to Branch access. A selected primary/default must be an active assignment; revoking it requires a replacement. No first/current/arbitrary Branch fallback exists.
- Authorization repair: the Branch-shell technical-account/operator architecture is preserved. Employee readiness, assignment, current Branch and authorization-version validation fail closed before operator verification or live session use. Effective permissions continue as active role grants plus direct grants minus active denials; no normal employee User login was added.
- Validation: selected Company/Branch/error/notification/employee Node coverage passed `28/28`; employee authorization static contracts, existing permission baseline (`128/128`), typecheck and targeted lint passed. The unique disposable PostgreSQL lifecycle passed and was dropped; no migration was required. Official `darfus_erp` remains `51/51/0` with no phase mutation.
- Runtime boundary: the administrator read-only session reached `/en/employees` but did not expose the create control, so no form was submitted. No approved employee credential exists; real employee Branch-shell runtime acceptance is blocked, not passed. Credentials and temporary wrapper files were removed.
- Runtime regression: the unchanged reuse-only harness otherwise retained accepted notification/module behavior, but observed two customer-financial requests before Branch-B READY during A→B. This is `FULL-REGRESSION-F007 = OPEN — BRANCH_TRANSITION_PRE_READY_FINANCIAL_TRAFFIC`; no repair was attempted here.
- Decision: `AUTHORIZATION-RUNTIME-FIX-CONT1 = PARTIAL`; F004/F005/F006 are `RESOLVED_BY_CODE_AND_ISOLATED_ACCEPTANCE`; employee runtime remains credential-blocked, and F007 is a separate release blocker. `RELEASE_READY = NO`; exact next marker `AUTHORIZATION-RUNTIME-FIX-CONT2`. Do not start it automatically.

## AUTHORIZATION-RUNTIME-AUDIT-CONT1 — 2026-07-29

- Checkpoint: `286a630` (`docs: accept full Product regression`). Read-only preflight retained main, 11 stashes, no remotes, protected semantic zero, required declaration hash, local runtime `3000/8000`, and database migrations `51/51/0`. No runtime process, migration, employee, Branch, role, permission or official data was changed.
- Findings: the Employee create modal initializes its required Branch label from Company session metadata, but submits no Branch ID. `POST /employees` only creates `EmployeeBranchAccess` when `branchId` exists. The dedicated Branch-access editor supports multiple explicit rows but does not select/synchronize a default; `employees.branch_id` is nullable and lacks a membership constraint. This proves `FULL-REGRESSION-F004` (unconsented primary-Branch preselection) and `F005` (operational mapping failure).
- Authorization model: active role permissions plus direct grants minus direct denials are calculated by the employee authorization service. Branch access is checked server-side, and authorization-version changes revoke stale operator sessions. However normal `/auth/login` authenticates User accounts; Employees have PIN/operator credentials consumed only through a Branch-shell account and active operator session. Employee creation does not create that account/link, proving `FULL-REGRESSION-F006` (employee login/Branch bootstrap contract incomplete).
- Runtime boundary: no approved employee credential was available. Employee runtime login is blocked, not passed; no credential was guessed or changed. Existing data aggregate checks found no duplicate active mapping or active primary/mapping mismatch. `RELEASE_READY = NO`; exact next marker: `AUTHORIZATION-RUNTIME-FIX-CONT1`. Do not start it automatically.

## FULL-REGRESSION-FIX-CONT1 — 2026-07-29

- Starting checkpoint: `a2fd04e` (`docs: record full regression Product findings`). Product commits `ca5e386`, `e2f075d`, `31a9fa3`, and `a32e42a` narrowly resolve list lifecycle ownership and Branch-aware Assets/Inventory reads. Harness-only commits `30129d3` and `a89deb7` make the existing read-only Branch-B asset proof explicit and restore the customer route for its pre-existing refresh check.
- Root causes/repair: Dashboard and Reservations had overlapping core owners; Suppliers combined core ownership with an imperative refetch; Purchase Orders varied a Company-only key by Branch generation. Core resource selection and stable declarative ownership remove duplicate authoritative completions. Assets/Inventory had an ungated direct path including `skipBranch`; it now waits for validated Branch READY, uses canonical transport, carries cancellation and Branch-key isolation.
- Validation/runtime: focused lifecycle, context and evidence suite passed `24/24`; typecheck passed; lint remained 0 errors/18 inherited warnings. The unchanged reuse-mode browser capture exited `0/PASS`, started no service child, retained no secret material, and passed all ten modules (pending and duplicate counts zero). N5/N8 each retained one bootstrap/Branch/list/unread/SSE lifecycle with zero observed auth/context/toast/reconnect counts. A→B and hard refresh passed; the explicit Branch-B Assets lifecycle retained both context booleans; logout retained zero protected traffic.
- Safety/decision: existing 3000/8000 services were preserved. Official local `darfus_erp` remained `51/51/0` with phase database mutation `0`; credentials and owned temporary material were removed. `FULL-REGRESSION-F002 = RESOLVED`, `FULL-REGRESSION-F003 = RESOLVED`, `FULL-REGRESSION = COMPLETE`, and `OPEN_RELEASE_BLOCKING_REGRESSIONS = 0`. `RELEASE_READY = NO`; Staging and Production remain unauthorized. Exact next marker: `MIGRATION-STAGING-REHEARSAL`; do not start automatically.

## FULL-REGRESSION-HARNESS-FIX-CONT1 — 2026-07-29

- Starting checkpoint: `af90f95` (`docs: record full regression coverage boundary`). Harness-only commits `d660ea8`, `9dec085`, `5b05719`, and `d54d3ba` add sanitized terminal module evidence without any Product, backend, package, database, migration or configuration change.
- Runtime: the accepted process-scoped, credential-clean replay reused the existing local `3000/8000` services and started no child. N5/N8, Branch A→B, notification/SSE and logout remain non-regressed; all observed N5/N8 context/auth/toast counters remain zero. No raw identity or payload was retained.
- Decision: `FULL-REGRESSION-F001 = RESOLVED`. `FULL-REGRESSION = PARTIAL` because read-only evidence proves `FULL-REGRESSION-F002` duplicate successful lifecycles (dashboard/suppliers/reservations/purchase orders) and `FULL-REGRESSION-F003` headerless Branch-scoped asset reads. Do not fix them in this phase. `RELEASE_READY = NO`; exact next marker: `FULL-REGRESSION-FIX-CONT1`.

## FULL-REGRESSION — bounded read-only result — 2026-07-29

- Starting checkpoint: `27c9259` (`docs: accept notification runtime lifecycle`), with 11 stashes, no remotes, no staged paths and only inherited protected CRLF-only artifacts. Required accepted history is present. The protected declaration retained its exact required hash.
- Safety/static result: official local `darfus_erp` was read-only at `51/51/0`, no idle transaction or waiting lock. The selected cross-domain Node suite passed `99/99`; permission baseline passed `128/128`; bootstrap/database guards, typecheck and full lint passed (0 errors, 18 inherited warnings). The real-PostgreSQL First Run test skipped without an explicit disposable target, preserving the official DB.
- Runtime result: the credential-clean unchanged reuse harness exited `0/PASS` against pre-existing `localhost:3000/8000`; it started/stopped no service. N5/N8 each had one bootstrap/Branch/list/unread/SSE lifecycle and zero observed 401/403/422, reconnects and notification error toasts. Branch A→B was atomic with zero pre-ready financial traffic; controlled Branch-B absence retained both headers and Branch READY; logout left no protected notification traffic. Wrapper and credential variables were absent afterward; secret leakage was zero.
- Decision: `FULL-REGRESSION = PARTIAL`, not because a Product failure is proven, but because the accepted harness does not retain sanitized per-module endpoint/status evidence for the complete browser matrix. `FULL-REGRESSION-F001 = OPEN — full module browser matrix not captured`; `RELEASE_READY = NO`; no RC, Staging or Production authority. Exact next marker: `FULL-REGRESSION-HARNESS-FIX-CONT1`. Do not start it automatically.

## NOTIF-ACCEPT — final notification/SSE reused-runtime acceptance — 2026-07-29

- Starting checkpoint: `1f44100` after the earlier runtime-availability boundary. The expected protected declaration was normalized from a generated dev-route import back to its exact HEAD hash before acceptance; no Product source was changed. Focused notification/Company/Branch/error/harness tests passed 50/50, typecheck and targeted lint passed, and production build was deferred to preserve the manually managed Next runtime.
- Secure run: an ACL-restricted external one-use wrapper held the temporary test credentials only in its own process environment. The unchanged `test:single-company-runtime` harness reused `localhost:3000/8000`, started no service child, exited `0/PASS`, removed owned temporary evidence, and left credentials, wrapper and wrapper directory absent. Secret and PII leakage counts are zero.
- N5/N8: each recorded one bootstrap, Branch bootstrap, notification list, unread count and SSE lifecycle; notification resources had Company context and no Branch context under the accepted Company-only contract. Observed 401/403/422, `SUPER_ADMIN_COMPANY_CONTEXT_REQUIRED`, reconnect and notification-error-toast counts were zero.
- Branch/logout: A→B observed non-ready before Branch-B READY, zero pre-ready financial requests, zero `BRANCH_CONTEXT_REQUIRED`, and zero new notification list/unread/SSE/toast lifecycle. The bounded Branch-B `RESOURCE_NOT_FOUND` retained both headers and Branch READY, so it remains a controlled Product-domain outcome rather than a notification failure. Refresh retained one notification lifecycle; logout returned normally with zero post-logout protected notification traffic.
- Decision: `NOTIF-ACCEPT = COMPLETE`; all listed notification lifecycle, SSE, toast ownership, domain compatibility and logout contracts pass. `NOTIF_ACCEPT_AUTHORIZED = CONSUMED`; `NOTIF-PRE1-CONT1-CONT1-CONT1-CONT1-F001 = RESOLVED`. Official DB remains `51/0`; no migration/data mutation, process control, deployment or push occurred. `RELEASE_READY = NO`; exact next marker is `FULL-REGRESSION`, not started automatically.

## NOTIF-ACCEPT — secure operator launch blocked by pre-existing runtime — 2026-07-29

- Starting checkpoint: `725e202`; Branch closure was reconciled in `00c1336` before notification acceptance. Focused notification/Company/Branch/error/harness validation passed 50/50, plus typecheck and targeted lint; the production build remains deferred to avoid manipulating a manually managed Next runtime.
- Secure launch: the external wrapper read credentials interactively, used only process-scoped variables, removed them in `finally`, self-removed, and yielded sanitized `exitCode: 1`, `reason: HARNESS_EXECUTION_FAILED`, no child, no fingerprint, no evidence, zero secret leakage and removed cleanup.
- Read-only diagnosis: immediately afterward, the required reused frontend 3000 and backend 8000 listeners/processes were absent; PostgreSQL remained healthy. This is an infrastructure availability boundary, not a Product or harness regression. No process, Product, DB, migration, package or configuration change was made.
- Decision: `NOTIF-ACCEPT = BLOCKED`; `BLOCKER = PREEXISTING_RUNTIME_UNAVAILABLE`; the notification finding remains open and `NOTIF_ACCEPT_AUTHORIZED` is unconsumed. Restore the accepted 3000/8000 runtime outside this phase, then rerun the unchanged reuse-mode capture. Do not start `FULL-REGRESSION`, deploy, or authorize Staging/Production.

## Branch context prerequisite closure — 2026-07-29

- Supplied evidence: an operator-mediated sanitized harness replay reused the existing local 3000/8000 runtime, exited `0/PASS`, and left no secret-bearing evidence. It completed N5/N8, safe existing-customer discovery, Branch-A invoice/statement/credit, normal A→B, controlled Branch-B profile absence, refresh and logout.
- Branch acceptance: the switch observed non-ready before Branch-B READY, emitted zero pre-ready financial requests and zero `BRANCH_CONTEXT_REQUIRED`. The Branch-A-only customer returned controlled `RESOURCE_NOT_FOUND` in Branch B with both context headers and no loop; Branch remained `READY`. This is a valid scoped-resource absence, not a Company/Branch regression.
- Decision: `BRANCH-CONTEXT-HARNESS-FIX-CONT1 = COMPLETE`; `BRANCH-CONTEXT-RUNTIME-FIX-CONT3-CONT1 = COMPLETE`; `BRANCH-CONTEXT-RUNTIME-FIX-CONT3 = COMPLETE`; `BRANCH-CONTEXT-RUNTIME-FIX = COMPLETE`; and `BRANCH-CONTEXT-RUNTIME-F001 = RESOLVED`. `NOTIF_ACCEPT_AUTHORIZED = YES` only for `NOTIF-ACCEPT`. `RELEASE_READY = NO`; no Staging/Production authority exists.

## BRANCH-CONTEXT-HARNESS-FIX-CONT1 — authenticated partial replay and safe Branch-B absence — 2026-07-29

- Starting checkpoint: `6fee077`. The supplied sanitized reused-runtime result started no process and passed its local fingerprint. N5 and N8 each observed exactly one bootstrap, Branch bootstrap, notification list, unread and SSE lifecycle; the Company gate was absent, scoped context was present where required, and observed 401/403/422, reconnect and notification-error-toast counts were zero.
- Customer evidence: a read-only existing customer was available in Branch A. Invoice, statement and credit each have one exact `200` response with Company/Branch context after exact correlation; short-lived aborted peers are separate and are not treated as successes. On ordinary A→B, that customer returned `404 RESOURCE_NOT_FOUND` with both headers and no `BRANCH_CONTEXT_REQUIRED`; the visible Branch state remained `READY`. It is a controlled Branch-scoped absence, not a Product context failure.
- Harness correction: `aa9e77a` records that controlled Branch-B profile absence without attempting its financial tabs, preserves context/Branch-ready checks, and attributes an error toast to notifications only when notification network evidence has failed. No Product, backend, package, migration, `.env`, data or runtime-process behavior changed. Focused helper tests, typecheck, targeted lint, diff check and the required Next declaration hash passed.
- Boundary: the prior run stopped before Branch-B refresh and logout. Re-run the unchanged authenticated harness through the approved operator-mediated wrapper; do not reuse its credentials elsewhere. `BRANCH-CONTEXT-RUNTIME-FIX = PARTIAL`, `NOTIF_ACCEPT_AUTHORIZED = NO`, and Staging/Production remain unauthorized.

## BRANCH-CONTEXT-HARNESS-FIX-CONT1 — 2026-07-29

- Starting checkpoint: `84631d9` (`docs: record customer financial Branch regression`). Harness-only commit `26d25a2` replaces ambiguous reverse method/path response pairing with a `WeakMap` keyed by the exact Playwright Request object. Each record has one idempotent terminal outcome; dynamic paths remain sanitized display/grouping data only.
- Regression coverage: out-of-order same-path statement success with aborted peer, independent same-path credit failure/success, late response after scenario change, duplicate terminal event, map cleanup and redaction all pass. The financial settle barrier now waits for at least one qualifying response per resource and zero pending short-lived financial records.
- Validation/safety: focused Branch/Company/notification/harness suites = 39 PASS; typecheck, targeted lint and `git diff --check` PASS. No Product/backend/package/migration/database change occurred, and existing 3000/8000 processes remain untouched. `NOTIF_ACCEPT_AUTHORIZED = NO`.
- Boundary: the operator must run the unchanged harness using only the temporary credential-free wrapper and process-scoped variables. Until its sanitized result is supplied, Branch A→B, Branch-B financial, refresh, notification compatibility and logout remain `NOT_OBSERVED`.

## BRANCH-CONTEXT-RUNTIME-FIX-CONT3-CONT1 — 2026-07-29

- Starting checkpoint: `3d04d0f` (`docs: record customer discovery evidence boundary`). The user supplied a sanitized, operator-mediated result from the unchanged repository harness; no credential, token, raw identifier, profile, HAR, screenshot, or Product change was retained.
- Runtime result: the harness reused (started no child) the verified local 3000/8000 services. Login succeeded. N5 and N8 each recorded one bootstrap/list/unread/SSE/Branch lifecycle; scoped Company context was present, the Company gate was absent, and observed 401/403/422, reconnect, notification-error-toast counts were zero. Read-only customer discovery found `MANY` and a safe visible profile route, confirming the earlier immediate post-navigation selector timing discrepancy.
- Boundary: Branch-A invoice, statement-v2 and credit all have a sanitized `200` record with Company/Branch context and zero `BRANCH_CONTEXT_REQUIRED`. The response collector is nevertheless ambiguous for concurrent same-method/same-normalized-path traffic: asynchronous callbacks select the same reverse-scanned pending entry and leave the older record `null`, making statement/credit summary success counts falsely zero. The harness aborts before Branch A→B, refresh and logout. This is a harness defect, not a Product regression.
- Validation: focused Branch/Company/notification/harness tests = 37 PASS; typecheck and targeted lint PASS; `git diff --check` PASS. Existing 3000/8000 processes were reused and untouched; the wrapper cleanup/result reports no secret leakage and cleanup `REMOVED`.
- Decision: `BRANCH-CONTEXT-RUNTIME-FIX-CONT3-CONT1 = PARTIAL`; `BRANCH-CONTEXT-RUNTIME-F001 = OPEN — harness response correlation prevents customer-financial acceptance`; `NOTIF_ACCEPT_AUTHORIZED = NO`; Staging/Production remain unauthorized. Exact next marker: `BRANCH-CONTEXT-HARNESS-FIX-CONT1`, limited to one-to-one request/response correlation, its exact concurrency test, and rerunning the same capture.

## BRANCH-CONTEXT-RUNTIME-FIX-CONT3 — 2026-07-29

- Starting checkpoint: `fa0d04a`. `013b388` is harness-only: customer discovery now waits for the existing list response and visible profile route, records only count categories/status/header booleans, and captures sanitized Branch A→B/refresh ordering. No Product, backend, package, migration, `.env`, data, or runtime-process change occurred.
- Boundary: the shell safety layer rejected both compliant process-scoped credential-launch attempts before the browser harness started. Browser credential entry was deliberately not used because it would violate the phase contract. Consequently there is no login, customer discovery, customer-financial A→B/refresh, notification, or logout runtime observation; the prior CONT1/CONT2 customer discrepancy is **NOT_PROVABLE** in this run. The likely immediate-DOM-probe deficiency is corrected, not asserted as the cause.
- Validation: Branch/Company/notification/error/harness-redaction suite = 37 passing; typecheck and targeted lint pass; production build deferred to preserve the pre-existing Next dev runtime. The official DB is unchanged at 51/0; the local 3000/8000 runtime remains reachable and untouched.
- Decision: `BRANCH-CONTEXT-RUNTIME-FIX-CONT3 = BLOCKED`, `BRANCH-CONTEXT-RUNTIME-F001 = OPEN — authenticated customer-financial evidence unavailable`, `NOTIF_ACCEPT_AUTHORIZED = NO`, Staging/Production unauthorized. Exact next marker: `BRANCH-CONTEXT-RUNTIME-FIX-CONT3-CONT1` — provide only a compliant process-scoped credential launch and run the unchanged deterministic harness.

## BRANCH-CONTEXT-RUNTIME-FIX-CONT2 — 2026-07-29

- Starting checkpoint: `e559d1f`. Product commits: `e79ac33` (atomic Branch transition), `29faffc` (transition ordering tests), `962bf44` (selection-required remains interactive), and `ba03581` (isolated transition runtime evidence). No backend, package, migration, `.env`, data or runtime-process change occurred.
- Root cause and repair: the prior provider cleared the API accessor while still publishing `READY`. The provider now enters `TRANSITIONING` first; a shared client guard aborts stale Branch-scoped work before fetch; only concrete Branch-keyed queries are cancelled; Branch B accessor/header is installed before `READY`; invoices, statements, credit and core reads receive React Query abort signals. Backend `BRANCH_CONTEXT_REQUIRED` remains unchanged.
- Validation: focused Branch/Company/notification/error/harness suites and typecheck pass. Targeted lint has zero errors and the two inherited customer-page image warnings. Production build remains deferred to avoid disturbing the manually running Next dev runtime.
- Reused browser evidence: N5 and N8 each observed exactly one bootstrap, Branch bootstrap, notification list, unread and SSE lifecycle; all observed 401/403/422, reconnect and notification-error-toast counts were zero. With five existing Branch choices, an isolated A→B window had Branch context on core scoped reads, `BRANCH_CONTEXT_REQUIRED = 0`, and new notification list/unread/SSE/error-toast counts all zero. Normal logout again produced zero protected notification traffic. Browser evidence/profile cleanup was removed and credential variables were absent afterward.
- Boundary: the authenticated UI now exposes no safe existing customer profile, so customer invoice/statement-v2/credit Branch-A→B and refresh evidence is `NOT_OBSERVED`. Do not create customer data or claim those counts. `BRANCH-CONTEXT-RUNTIME-FIX-CONT2 = PARTIAL`, `BRANCH-CONTEXT-RUNTIME-F001 = OPEN — customer-financial A→B evidence unavailable`, `NOTIF_ACCEPT_AUTHORIZED = NO`, Staging/Production remain unauthorized. The official local DB is unchanged at 51 applied / 0 pending with zero phase mutation/idle/waiting locks. Exact next marker: `BRANCH-CONTEXT-RUNTIME-FIX-CONT3`.

## BRANCH-CONTEXT-RUNTIME-FIX-CONT2-CONT1 — 2026-07-28

- The user confirmed a manual local migration command before this phase. The prior official historical baseline was 50 applied / 1 pending; it is not rewritten. Read-only reconciliation now adopts `darfus_erp` / `public` on local PostgreSQL 5432 as **51 applied / 0 pending**. Reconciliation itself made zero database mutations and no rollback is required.
- Migration 51 is `20260728010000-create-first-run-setup-state.js`, SHA-256 `D9D576E89625A78402C0DD06570104905A1E7D6CDB192D89E42692237101A024`. Its schema-only `first_run_setup_states` table has all nine declared columns, expected primary key, no invalid index, zero marker rows, registry occurrence one, and exact 51/51 source/registry set equality.
- Current frontend/backend identities are accepted: workspace Next serves 3000; workspace Nodemon/server serves 8000; the backend's replacement PID has a local 5432 connection and DB health is UP. The process replacement is accepted as same-project runtime identity, not a restart performed by this phase.
- Locks are clean (idle-in-transaction/waiting = 0); credentials are absent. `BRANCH-CONTEXT-RUNTIME-FIX-CONT2 = AUTHORIZED_TO_RESUME`, but has not started. `NOTIF_ACCEPT_AUTHORIZED = NO`; Staging and Production remain unauthorized. Exact next marker: `BRANCH-CONTEXT-RUNTIME-FIX-CONT2`.

## BRANCH-CONTEXT-RUNTIME-FIX-CONT1 — 2026-07-28

- Start/final baseline: `main` at `5bec160` (`docs: record Branch context runtime boundary`); 11 stashes, no remotes, protected files semantic-zero and `next-env.d.ts` exact. The existing local frontend/backend were reused at their original 3000/8000 PIDs; neither was started, stopped or targeted for cleanup. Official `darfus_erp` remained 50 applied / 1 pending source migration with an unchanged aggregate fingerprint and zero idle/waiting locks.
- A safe UI-discovered `CUSTOMER_A` was available. With Company/Branch `READY`, read-only invoice, statement-v2 and credit each made one `200` request with Company and Branch context present; no pre-ready customer-financial request or `BRANCH_CONTEXT_REQUIRED` appeared in that initial profile lifecycle. No customer, financial, Branch or Company data was changed and no identifying data was retained.
- The normal customer-profile Branch A→B transition is a Product regression. `selectBranch` clears the canonical Branch accessor/cache before the provider exits `READY`; React Query can therefore start enabled financial requests with no active accessor. The observed run had three `422 BRANCH_CONTEXT_REQUIRED` responses and ended with Branch state `INVALID`. Do not patch in this phase, weaken middleware, or claim refresh-on-profile/logout acceptance.
- The bounded read-only capture waited for customer-list rendering and financial response status, so it did not mistake an asynchronously rendered safe profile or an in-flight `200` for absent evidence. It recorded only normalized paths, statuses and header-presence booleans.
- Focused checks: 26 PASS; typecheck PASS; targeted lint 0 errors with two inherited `no-img-element` warnings. Credential variables were process-scoped and absent after browser cleanup; browser temp cleanup was `REMOVED`; no secret/PII was retained. `NOTIF_ACCEPT_AUTHORIZED = NO`. Exact next marker: `BRANCH-CONTEXT-RUNTIME-FIX-CONT2`, limited to making the Branch provider non-ready before the old accessor/cache is cleared and then re-running customer-profile refresh/A→B/logout evidence.
