...
function makeResult<T>(data: T, source: "LIVE" | "LOCAL" | "CACHE" = "LOCAL"): DashboardWidgetResult<T> {
  return {
    data,
    source,
    ok: true,
...
export class LocalDashboardProvider implements DashboardDataProvider {
  ...
  async getGoldPrice(ctx: DashboardQueryContext): Promise<DashboardWidgetResult<DashboardGoldData>> {
    ...
    const updatedAt = goldPrice.updatedAt ?? "";
    const ageMs = updatedAt ? Date.now() - new Date(updatedAt).getTime() : Number.POSITIVE_INFINITY;
    const isStale = goldPrice.status === "STALE" || ageMs > (goldPrice.staleAfterSeconds ?? 120) * 1000;
    ...
    const data: DashboardGoldData = {
      updatedAt,
      updatedBy: goldPrice.updatedBy ?? "LIVE_PROVIDER",
      prices,
      trend: "FLAT",
      changePercent: null,
      isStale,
    ...
    return makeResult(data, goldPrice.source === "CANONICAL_GOLD_MARKET_QUOTE" ? "LIVE" : "LOCAL");
  }
