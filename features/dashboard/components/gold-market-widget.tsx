...
export function GoldMarketWidget({
  ...
  const fmt = (n: number) => formatEnglishNumber(n, { maximumFractionDigits: 8 });
  ...
  const updatedLabel = data?.isStale
    ? t("goldPriceStale")
      : data?.updatedAt
      ? t("goldLastUpdate", { time: formatTime(data.updatedAt, "Asia/Dubai", locale) })
      : t("goldLastUpdateNow");
        ...
        <div>
          ...
          {karat24Price > 0 ? (
            <div className="mt-2 flex items-baseline gap-2">
              <NumericToken className="text-3xl font-black">{fmt(karat24Price)}</NumericToken>
              ...
              <span className={cn("flex items-center gap-1 text-xs font-bold", trendColor)}>
                ...
                {data?.changePercent ? <NumericToken>{`${data.changePercent > 0 ? "+" : ""}${data.changePercent.toFixed(2)}%`}</NumericToken> : ""}
              </span>
            </div>
          ) : (
        ...
        >
          {prices
            ...
            .map((entry) => (
              ...
              >
                <p className="text-[10px] font-semibold text-brand-200"><NumericToken>{`${entry.karat}K`}</NumericToken></p>
                <NumericToken className="mt-1 text-sm font-black">{fmt(entry.pricePerGram)}</NumericToken>
