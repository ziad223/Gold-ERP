...
export class LocalCustomerRepository implements CustomerRepository {
  ...
  async create(customer: CustomerCreatePayload): Promise<MutationResult<Customer>> {
    const customerId = nextPrefixedId(this.ctx.customers.map((c) => c.id), "CUS", 4);
    ...
    const newCustomer: Customer = {
      ...
      status: "active",
      attachments: [],
      kycDetails: { status: "not-started", amlStatus: "clear" },
    };
  ...
  async update(id: string, updates: CustomerUpdatePayload): Promise<MutationResult<Customer>> {
