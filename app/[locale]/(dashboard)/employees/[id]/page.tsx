...
import { formatDateTime as formatUiDateTime } from "@/lib/dates/dates";
...
function formatDate(value?: string | null) {
  if (!value) return "—";
  return formatUiDateTime(value, "Asia/Dubai", "ar");
}
        ...
        <div className="mt-5 space-y-4 text-xs">
          {logs.map((log: any) => (
            <div key={log.id} className="space-y-1 border-l-2 border-brand-500 py-1 pl-4 dark:border-brand-400">
              <div className="flex justify-between"><span className="font-bold capitalize text-navy-900 dark:text-white">{log.action}</span><span className="text-[10px] text-slate-400">{formatDate(log.date || log.createdAt)}</span></div>
