      ...
      <Card className="p-4 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          ...
          <label className="block">
            ...
            <DateInput className="input-base mt-1" value={filterCreatedFrom} onChange={setFilterCreatedFrom} />
          </label>
          <label className="block">
            <span className="text-[10px] font-bold text-muted">{rtl ? "تاريخ الحجز إلى" : "Created To"}</span>
            <DateInput className="input-base mt-1" value={filterCreatedTo} onChange={setFilterCreatedTo} />
          </label>
          <label className="block">
            <span className="text-[10px] font-bold text-muted">{rtl ? "تاريخ الانتهاء من" : "Expiry From"}</span>
            <DateInput className="input-base mt-1" value={filterExpiryFrom} onChange={setFilterExpiryFrom} />
          </label>
          <label className="block">
            <span className="text-[10px] font-bold text-muted">{rtl ? "تاريخ الانتهاء إلى" : "Expiry To"}</span>
            <DateInput className="input-base mt-1" value={filterExpiryTo} onChange={setFilterExpiryTo} />
          </label>
          <label className="block">
            ...
            <Card key={reservation.id} className="p-5 flex flex-col justify-between space-y-4 hover:-translate-y-0.5 hover:border-brand-500/30 hover:shadow-panel transition">
              <div>
                ...
                <div className="mt-4 space-y-2 text-xs border-t border-dashed border-border pt-3">
                  ...
                  <p className="flex items-center gap-1.5 text-muted"><Calendar className="h-3.5 w-3.5" />{formatDateTime(reservation.expiresAt, "Asia/Dubai", locale)}</p>
        ...
        {selectedReservation ? (
          <div className="space-y-4 text-sm">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Info label={rtl ? "تاريخ الانتهاء" : "Expiry"} value={formatDateTime(selectedReservation.expiresAt, "Asia/Dubai", locale)} />
            ...
            <Section title={rtl ? "سجل التعديلات" : "Amendment history"}>
              {(selectedReservation.amendments ?? []).length === 0 ? (
                ...
                <div key={amendment.id} className="rounded-2xl border border-border p-3 text-xs space-y-1">
                  ...
                  <p>{rtl ? "السبب" : "Reason"}: {amendment.reason || "—"} · {amendment.createdBy || "—"} · {formatDateTime(amendment.createdAt, "Asia/Dubai", locale)}</p>
                </div>
              ))}
            </Section>
            ...
            <Section title={rtl ? "سجل التمديد" : "Extension history"}>
              {(selectedReservation.expiryExtensions ?? []).length === 0 ? (
                ...
                <div key={extension.id} className="rounded-2xl border border-border p-3 text-xs">
                  {formatDateTime(extension.oldExpiry, "Asia/Dubai", locale)} → {formatDateTime(extension.newExpiry, "Asia/Dubai", locale)} · {extension.reason || "—"} · {extension.extendedBy || "—"}
                </div>
              ))}
            </Section>
              ...
              <Section title={rtl ? "سجل التدقيق" : "Audit timeline"}>
                {reservationAuditQuery.isFetching ? (
                  ...
                  <div key={event.id} className="rounded-2xl border border-border p-3 text-xs space-y-1">
                    <p className="font-bold">{event.action} · {event.user || "—"} · {formatDateTime(event.date, "Asia/Dubai", locale)}</p>
        ...
        <form onSubmit={handlePostReservation} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <label className="block col-span-2">
              ...
              <DateInput required className="input-base" value={expDate} onChange={setExpDate} />
            </label>
          </div>
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block">
                ...
                <DateInput className="input-base" value={renewExpiry} onChange={setRenewExpiry} />
              </label>
              <label className="block">
