...
export default function CustomersPage() {
  ...
  const { items: customers, loading, error, query, setQuery, refresh } = useCustomers({
    page: 1,
    pageSize: 100,
    search: queryState,
    filters: {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      if (isEdit) {
        ...
        const generatedId = `CUS-${String(customers.length + 61).padStart(4, "0")}`;
