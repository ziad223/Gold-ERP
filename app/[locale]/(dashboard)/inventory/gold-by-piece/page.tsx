...
type Draft = {
  ...
  condition: string; supplierId: string; purchaseDate: string; makingPerGram: string;
  currentMakingPerGram: string; markupPercent: string; maximumDiscountPercent: string;
  itemCode: string; locationId: string; rfid: string; notes: string; purchaseGoldRate: string;
};

const initialDraft: Draft = {
  description: "", goldColor: "", karat: "21", grossWeight: "", stoneWeight: "0", condition: "NEW",
  supplierId: "", purchaseDate: new Date().toISOString().slice(0, 10), makingPerGram: "",
  currentMakingPerGram: "", markupPercent: "", maximumDiscountPercent: "", itemCode: "",
  locationId: "", rfid: "", notes: "", purchaseGoldRate: "",
};
...
export default function GoldByPieceProfilePage() {
  ...
  const load = async (quiet = false) => {
    ...
    setError("");
    try {
      const response = await apiClient<any>("/inventory-v2/gold-by-piece/contract", { locale, branchId: branchId || undefined });
  ...
  useEffect(() => {
    ...
    if (supplier) setDraft((current) => current.supplierId ? current : { ...current, supplierId: supplier.id });
  ...
  const submit = async () => {
    if (!branchId || !draft.supplierId || !preview) { setError(rtl ? "المورد والفرع والمعاينة مطلوبة قبل الاستلام." : "Supplier, branch, and server preview are required."); return; }
    setSubmitLoading(true); setError(""); setResult(null);
    try {
      const barcode = contract?.barcode || { inventoryCodes: [], itemCodes: [] };
      const inventoryCode = barcode.inventoryCodes?.find((entry: any) => entry.assetType === "gold-piece" && entry.isActive)?.code;
      const itemCode = draft.itemCode || barcode.itemCodes?.find((entry: any) => entry.isActive && entry.isClientApproved !== false && (!inventoryCode || !entry.allowedInventoryCodes?.length || entry.allowedInventoryCodes.includes(inventoryCode)))?.code;
      if (!itemCode) throw new Error(rtl ? "كود الباركود المعتمد مطلوب." : "An approved barcode item code is required.");
      const idempotencyKey = generateUUID();
      const piece = { ...item, type: "gold-piece", category: "Gold By Piece", inventoryCode, itemCode, locationId: draft.locationId || undefined, rfid: draft.rfid || undefined, notes: draft.notes || undefined, pricing: { markupPercent: num(draft.markupPercent), maximumDiscountPercent: draft.maximumDiscountPercent === "" ? undefined : num(draft.maximumDiscountPercent) } };
      const response = await apiClient<any>("/purchase-orders/receive", { method: "POST", locale, branchId, idempotencyKey, body: JSON.stringify({ supplierId: draft.supplierId, warehouseId: branchId, branchId, purchaseDate: draft.purchaseDate, paymentMethod: "credit", paidAmount: 0, applyVat: contract?.vat?.enabled !== false, inventoryV2: true, idempotencyKey, items: [{ name: draft.description, description: draft.description, type: "gold-piece", category: "Gold By Piece", inventoryProfile: "GOLD_BY_PIECE", quantity: 1, weightPerUnit: num(draft.grossWeight), grossWeight: num(draft.grossWeight), unitCost: num(preview.purchase?.totalPurchaseCost), perPiece: [piece] }] }) });
  ...
  return <div dir={rtl ? "rtl" : "ltr"} className="space-y-6 text-xs">
    ...
    <Section number={5} title={rtl ? "الهوية التشغيلية" : "Operational Identity"}><><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><SelectField label={rtl ? "كود الباركود" : "Barcode Item Code"} value={draft.itemCode} onChange={(value) => update("itemCode", value)} options={(contract?.barcode?.itemCodes || []).filter((entry: any) => entry.isActive && entry.isClientApproved !== false).map((entry: any) => ({ value: entry.code, label: `${entry.code} — ${entry.displayName}` }))} required /><SelectField label={rtl ? "الموقع" : "Location"} value={draft.locationId} onChange={(value) => update("locationId", value)} options={(contract?.locations || []).map((entry: any) => ({ value: entry.id, label: `${entry.code} — ${entry.name}` }))} /><Field label="RFID" value={draft.rfid} onChange={(value) => update("rfid", value)} /><Field label={rtl ? "الفرع" : "Branch"} value={branchId || "—"} readOnly /></div><Field label={rtl ? "ملاحظات" : "Notes"} value={draft.notes} onChange={(value) => update("notes", value)} /></></Section>
    ...
    <div className="flex justify-end gap-3"><Button variant="secondary" onClick={() => setDraft(initialDraft)}>{rtl ? "مسح" : "Clear"}</Button><Button onClick={() => void submit()} disabled={submitLoading || previewLoading || !healthReady || !preview || !draft.supplierId || !draft.description || !draft.itemCode}><Coins className="h-4 w-4" />{submitLoading ? (rtl ? "جارٍ الاستلام…" : "Receiving…") : (rtl ? "استلام أصل واحد" : "Receive one Asset")}</Button></div>
  </div>;
}
