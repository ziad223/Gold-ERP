...
type Draft = {
  ...
  supplierId: string; purchaseDate: string; grossWeight: string; stoneWeight: string; stoneName: string;
  itemCode: string; purchaseGoldRate: string; makingPerGram: string; currentMakingPerGram: string; certificateCost: string;
  sellingMakingPerGram: string; minimumMakingPerGram: string; locationId: string; rfid: string;
};

const initialDraft: Draft = {
  strategy: "GOLD_BY_WEIGHT_JEWELLERY", description: "", karat: "21", goldColor: "", brand: "", model: "", modelNumber: "",
  supplierId: "", purchaseDate: new Date().toISOString().slice(0, 10), grossWeight: "", stoneWeight: "0", stoneName: "",
  itemCode: "", purchaseGoldRate: "", makingPerGram: "", currentMakingPerGram: "", certificateCost: "", sellingMakingPerGram: "", minimumMakingPerGram: "", locationId: "", rfid: "",
};
...
export default function GoldByWeightProfilePage() {
  ...
  const itemPayload = useMemo(() => ({
    ...
    supplierId: draft.supplierId || undefined, purchaseDate: draft.purchaseDate, itemCode: draft.itemCode || undefined, locationId: draft.locationId || undefined,
  ...
  const loadContract = async (quiet = false) => {
    ...
    setError("");
    try {
      const response = await apiClient<any>("/inventory-v2/gold-by-weight/contract", { locale, branchId: branchId || undefined });
  ...
  useEffect(() => {
    ...
    setDraft((current) => current.supplierId ? current : { ...current, supplierId: accessibleSupplier.id });
  ...
  const submit = async () => {
    if (!contract || !branchId || !draft.supplierId) { setError(rtl ? "المورد والفرع مطلوبان للاستلام القانوني." : "Supplier and branch are required for canonical receiving."); return; }
    if (!preview) { setError(rtl ? "انتظر المعاينة القانونية قبل الحفظ." : "Wait for the canonical server preview before submitting."); return; }
    setSubmitLoading(true); setError(""); setSubmitResult(null);
    const idempotencyKey = generateUUID();
    try {
      const barcode = contract.barcode || { inventoryCodes: [], itemCodes: [] };
      const inventoryCode = barcode.inventoryCodes.find((item: any) => item.assetType === "gold-weight" && item.isActive)?.code;
      const itemCode = draft.itemCode || barcode.itemCodes.find((item: any) => item.isActive && item.isClientApproved !== false && (!inventoryCode || !item.allowedInventoryCodes?.length || item.allowedInventoryCodes.includes(inventoryCode)))?.code;
      const unitCost = number(preview.purchase?.totalPurchaseCost);
      const piece = { ...itemPayload, profile: draft.strategy, inventoryProfile: draft.strategy, type: "gold-weight", category: "Gold By Weight", inventoryCode, itemCode, weightPerUnit: number(draft.grossWeight), unitCost, cost: unitCost, goldValuation: {
        purchaseGoldRate: draft.purchaseGoldRate ? number(draft.purchaseGoldRate) : undefined,
      ...
      const response = await apiClient<any>("/purchase-orders/receive", {
        ...
        body: JSON.stringify({ supplierId: draft.supplierId, warehouseId: branchId, branchId, purchaseDate: draft.purchaseDate, paymentMethod: "credit", paidAmount: 0, inventoryV2: true, idempotencyKey, items: [{
          ...
          inventoryCode, itemCode, quantity: 1, weightPerUnit: number(draft.grossWeight), grossWeight: number(draft.grossWeight), unitCost, perPiece: [piece],
        }] }),
      });
  ...
  return <div dir={rtl ? "rtl" : "ltr"} className="space-y-6 text-xs">
    ...
    <Section number={3} title={rtl ? "بيانات الشراء" : "Purchase Information"}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><SelectField label={rtl ? "المورد" : "Supplier"} value={draft.supplierId} onChange={(value) => update("supplierId", value)} options={(contract?.suppliers || []).map((item) => ({ value: item.id, label: item.name }))} required /><Field label={rtl ? "سعر الذهب وقت الشراء / g" : "Global Gold Rate At Purchase / g"} value={draft.purchaseGoldRate} onChange={(value) => update("purchaseGoldRate", value)} type="number" step="0.00000001" /><Field label={rtl ? "المصنعية / g" : "Making Cost Per Gram"} value={draft.makingPerGram} onChange={(value) => update("makingPerGram", value)} type="number" step="0.00000001" required={draft.strategy !== "GOLD_BAR_24K"} disabled={draft.strategy === "GOLD_BAR_24K"} /><Field label={rtl ? "تكلفة الشهادة" : "Certificate Cost"} value={draft.certificateCost} onChange={(value) => update("certificateCost", value)} type="number" step="0.01" required={draft.strategy === "GOLD_BAR_24K"} disabled={draft.strategy !== "GOLD_BAR_24K"} /></div><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Field label={rtl ? "قيمة الذهب" : "Total Gold Value"} value={money(preview?.purchase?.goldValue, contract?.currency || "AED", locale)} readOnly /><Field label={rtl ? "إجمالي المصنعية" : "Total Making Cost"} value={money(preview?.purchase?.makingTotal, contract?.currency || "AED", locale)} readOnly /><Field label="VAT" value={money(preview?.purchase?.vatAmount, contract?.currency || "AED", locale)} readOnly /><Field label={rtl ? "إجمالي الشراء" : "Total Purchase Cost"} value={money(preview?.purchase?.totalPurchaseCost, contract?.currency || "AED", locale)} readOnly /></div></Section>
    ...
    <Section number={7} title={rtl ? "الحالة والفرع والموقع" : "Status / Condition / Branch / Location"}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Field label={rtl ? "الفرع" : "Branch"} value={branchId || "—"} readOnly /><SelectField label={rtl ? "الموقع" : "Location"} value={draft.locationId} onChange={(value) => update("locationId", value)} options={(contract?.locations || []).map((item) => ({ value: item.id, label: `${item.code} — ${item.name}` }))} /><Field label={rtl ? "الحالة التشغيلية" : "Operational Status"} value="AVAILABLE after canonical receive" readOnly /><Field label={rtl ? "الحالة" : "Condition"} value="—" readOnly /></div><p className="text-[10px] text-slate-500">{rtl ? "الفرع من سياق الخادم؛ الحالة لا تُرسل من الشاشة ولا تتحول إلى Product quantity." : "Branch is server-context-owned; operational status is not intake-editable and never becomes Product quantity."}</p></Section>
    ...
    <div className="flex flex-wrap justify-end gap-3"><Button variant="secondary" onClick={() => setDraft(initialDraft)}>{rtl ? "مسح" : "Clear"}</Button><Button onClick={() => void submit()} disabled={submitLoading || !healthReady || masterMissing || !preview || !draft.supplierId || !draft.description || !draft.itemCode}><Save className="h-4 w-4" />{submitLoading ? (rtl ? "جارٍ الاستلام…" : "Receiving…") : (rtl ? "استلام قطعة واحدة" : "Receive one Asset")}</Button></div>
  </div>;
}
