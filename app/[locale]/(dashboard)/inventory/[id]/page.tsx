...
const STATUS_LABELS_EN: Record<string, string> = { AVAILABLE: "Available", RESERVED: "Reserved", SOLD: "Sold", PENDING_TRANSFER: "Pending transfer", WORKSHOP: "In workshop", RETURNED: "Returned", MISSING: "Missing", MELTED: "Melted", REVERSAL_PENDING: "Reversal pending", REVERSED: "Reversed" };
...
const LIFECYCLE_PATH_LABELS: Record<string, { ar: string; en: string; ownerAr: string; ownerEn: string }> = {
  AVAILABLE: { ar: "متاح عبر مسار المراجعة/الإرجاع", en: "Available through return/review path", ownerAr: "مراجعة المرتجعات", ownerEn: "Return review" },
  RESERVED: { ar: "الحجز", en: "Reservation", ownerAr: "مسار الحجوزات", ownerEn: "Reservation workflow" },
  PENDING_TRANSFER: { ar: "تحويل الفروع", en: "Branch transfer", ownerAr: "مسار التحويلات", ownerEn: "Transfer workflow" },
  WORKSHOP: { ar: "الورشة", en: "Workshop", ownerAr: "مسار الورشة", ownerEn: "Workshop workflow" },
  SOLD: { ar: "البيع", en: "Sale", ownerAr: "نقطة البيع", ownerEn: "POS" },
  MISSING: { ar: "بلاغ فقد", en: "Missing report", ownerAr: "إجراء الفقد", ownerEn: "Missing-case workflow" },
  MELTED: { ar: "الصهر/التصنيع", en: "Melting/manufacturing", ownerAr: "التصنيع", ownerEn: "Manufacturing workflow" },
  RETURNED: { ar: "مراجعة المرتجع", en: "Returned-item review", ownerAr: "مراجعة المرتجعات", ownerEn: "Return review" },
  REVERSAL_PENDING: { ar: "تعليق عكس شراء ذهب العميل", en: "Customer-gold reversal hold", ownerAr: "مسار CGP", ownerEn: "CGP workflow" },
  REVERSED: { ar: "عكس شراء ذهب العميل", en: "Customer-gold reversal", ownerAr: "مسار CGP", ownerEn: "CGP workflow" },
};
const EVENT_LABELS: Record<string, { ar: string; en: string }> = {
  TRANSFER_REQUEST: { ar: "طلب نقل", en: "Transfer requested" },
  TRANSFER_OUT: { ar: "تم إرسال الأصل للنقل", en: "Transfer dispatched" },
  TRANSFER_IN: { ar: "تم استلام الأصل من النقل", en: "Transfer received" },
  WORKSHOP_SENT: { ar: "تم الإرسال إلى الورشة", en: "Sent to workshop" },
  WORKSHOP_RETURNED: { ar: "تمت الإعادة من الورشة", en: "Returned from workshop" },
  PURCHASE_RECEIVED: { ar: "تم استلام الشراء", en: "Purchase received" },
  RFID_ASSIGNED: { ar: "تم إسناد RFID", en: "RFID assigned" },
  RFID_REPLACED: { ar: "تم استبدال RFID", en: "RFID replaced" },
  RFID_UNASSIGNED: { ar: "تم إلغاء ربط RFID", en: "RFID unassigned" },
};

const text = (value: unknown) => value === null || value === undefined || value === "" ? "—" : String(value);
const dateTime = (value: unknown, locale: string) => value ? formatDateTime(String(value), "Asia/Dubai", locale) : "—";
const lifecycleState = (value: unknown, rtl: boolean) => {
  const raw = String(value ?? "");
  const key = raw.startsWith("operational_status:") ? raw.slice("operational_status:".length) : raw;
  return (rtl ? STATUS_LABELS[key] : STATUS_LABELS_EN[key]) || raw || "—";
};
const eventLabel = (value: unknown, rtl: boolean) => {
  const key = String(value ?? "");
  return EVENT_LABELS[key]?.[rtl ? "ar" : "en"] || (rtl ? "حدث تشغيلي" : "Operational event");
};
function Field({ label, value }: { label: string; value: unknown }) { return <div className="rounded-xl bg-slate-50 p-3 dark:bg-navy-950"><p className="text-[11px] font-semibold text-slate-400">{label}</p><p className="mt-1 break-words text-sm font-bold tabular-nums text-navy-900 dark:text-slate-100" dir="auto">{text(value)}</p></div>; }
...
export default function AssetDetailsPage() {
  ...
  const lifecyclePaths = (data.legalActions || []).map((action: string) => LIFECYCLE_PATH_LABELS[action]).filter(Boolean);

  return <div className="space-y-5 text-sm">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><Link href="/inventory" className="mb-3 inline-flex items-center gap-1 font-bold text-slate-400 underline-offset-4 hover:text-brand-700 hover:underline"><BackIcon className="h-4 w-4" />{rtl ? "كل القطع" : "All Items"}</Link><div className="flex flex-wrap items-center gap-2"><h1 className="text-2xl font-black text-navy-950 dark:text-white">{asset.description || asset.name}</h1><Badge tone={STATUS_TONES[asset.operationalStatus] || "slate"}>{lifecycleState(asset.operationalStatus, rtl)}</Badge></div><p className="mt-2 font-mono text-[11px] text-slate-400" dir="ltr">{asset.id} · {asset.barcode}</p></div><Badge tone="violet">{PROFILE_LABELS[profile]?.[rtl ? "ar" : "en"] || profile}</Badge></div>

    <Section title={rtl ? "حالة المخزون" : "Stock Status"}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label={rtl ? "الحالة التشغيلية" : "Operational status"} value={lifecycleState(asset.operationalStatus, rtl)} /><Field label={rtl ? "الفرع" : "Branch"} value={asset.branch || asset.branchId} /><Field label={rtl ? "الموقع" : "Location"} value={asset.location || asset.locationId} /><Field label={rtl ? "تاريخ الحالة/آخر حدث" : "Status date / latest event"} value={dateTime(timeline[0]?.occurredAt, locale)} /></div><p className="mt-3 text-[11px] text-slate-500">{rtl ? "الحالة للعرض فقط؛ تغييرها لا يتم من حقول الإدخال، بل من المسار القانوني للحركة." : "Status is read-only here; canonical business actions, not intake fields, change it."}</p></Section>
