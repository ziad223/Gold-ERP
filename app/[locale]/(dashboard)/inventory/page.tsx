...
export default function InventoryPage() {
  ...
  const filteredProducts = useMemo(() => {
    return filterData(
      products,
      query,
      [(item) => item.productCode, (item) => item.productName, (item) => item.description, (item) => item.branchName],
      [(item) => type === "all" || item.stockType === type, (item) => branch === "all" || item.branchName === branch]
    );
  }, [products, query, type, branch]);

  const filteredAssets = useMemo(() => {
    const activeAssets = assets.filter((a) => !a.parentAssetId);
    return filterData(
      activeAssets,
      query,
      [(item) => item.id, (item) => item.name, (item) => item.barcode, (item) => item.rfid, (item) => item.category, (item) => item.branch],
      [(item) => type === "all" || item.type === type, (item) => statusFilter === "all" || item.status === statusFilter, (item) => branch === "all" || item.branch === branch]
    );
  }, [assets, query, type, statusFilter, branch]);

  const stats = useMemo(() => {
    if (activeTab === "products") {
      const activeProducts = products.filter((p) => p.isActive);
      const totalCount = activeProducts.length;
      const totalValue = activeProducts.reduce((sum, p) => sum + (Number(p.averageCost) || 0) * (Number(p.quantityOnHand) || 0), 0);
      const totalWeight = activeProducts.reduce((sum, p) => sum + (Number(p.totalWeight) || 0), 0);
      const reservedCount = activeProducts.reduce((sum, p) => sum + (Number(p.quantityReserved) || 0), 0);
      return { totalCount, totalValue, totalWeight, reservedCount };
    } else {
      const activeAssets = assets.filter((item) => item.status !== "sold" && item.status !== "archived" && !item.parentAssetId);
      const totalCount = activeAssets.length;
      const totalValue = activeAssets.reduce((sum, item) => sum + (Number(item.cost) || 0), 0);
      const totalWeight = activeAssets.reduce((sum, item) => sum + (Number(item.netWeight) || 0), 0);
      const reservedCount = assets.filter((item) => item.status === "reserved" && !item.parentAssetId).length;
      return { totalCount, totalValue, totalWeight, reservedCount };
    }
  }, [products, assets, activeTab]);
    ...
    <div className="space-y-6 text-xs">
      ...
      <Card className="overflow-hidden">
        <DataToolbar
          ...
          resultCount={activeTab === "products" ? filteredProducts.length : filteredAssets.length}
