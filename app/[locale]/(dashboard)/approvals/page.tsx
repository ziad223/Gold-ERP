...
import { cgpGovernanceLabel, cgpBusinessStatusLabel } from "@/lib/cgp/presentation";
    ...
    <div className="space-y-6">
      ...
      {canReviewGold && <section className="space-y-3">
        ...
        {goldRequests.length === 0 ? <Card className="p-6 text-center text-xs text-muted">{rtl ? "لا توجد طلبات شراء ذهب معلقة ضمن نطاقك." : "No scoped Gold Purchase approvals are pending."}</Card> : goldRequests.map((request) => {
          ...
          const actionabilityBlocked = request.actionable === false;
          const immutableLinkedDocument = request.actionBlockedCode === "DOCUMENT_IMMUTABLE";
          const linkedStatusLabel = request.linkedBusinessStatus === "REVERSED"
            ? (rtl ? "معكوس" : "Reversed")
            : request.linkedBusinessStatus === "POSTED"
              ? (rtl ? "تم ترحيله" : "Posted")
              : request.linkedBusinessStatus;
           const aggregateLabel = request.aggregateType === "cgp" ? (rtl ? "شراء ذهب من عميل" : "Customer Gold Purchase") : (rtl ? "شراء ذهب استثماري" : "Investment Gold Purchase");
           return <Card key={request.id} className="space-y-4 p-5">
             <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs text-muted" dir="ltr">{request.id}</p><h3 className="font-black" dir="ltr">{snapshot.documentNumber || request.documentId}</h3><p className="text-xs text-muted">{aggregateLabel} · {snapshot.reference?.name || "—"} · {snapshot.items?.length || 0} {rtl ? "بند" : "lines"}</p></div><Badge tone="amber">{cgpGovernanceLabel(request.approvalStatus, locale)}</Badge></div>
