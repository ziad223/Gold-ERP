        ...
        <Card className="overflow-hidden">
          ...
          {error ? (
            ...
            <div className={loading ? "opacity-60 transition-opacity" : "transition-opacity"}>
              <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-start text-xs"><thead className="bg-table-header text-muted"><tr><th className="px-5 py-4">{t("entry")}</th><th className="px-5 py-4">{t("descriptionCol")}</th><th className="px-5 py-4">{t("amount")}</th><th className="px-5 py-4">{t("status")}</th><th className="px-5 py-4">{common("actions")}</th></tr></thead><tbody className="divide-y divide-border">{rows.map((entry) => { const badge = statusBadge(entry); return <tr key={entry.id} className="hover:bg-table-row-hover"><td className="px-5 py-4 font-mono font-bold text-brand-700 dark:text-brand-300">{entry.id}</td><td className="px-5 py-4 font-bold">{entry.description}<p className="mt-1 text-[10px] font-normal text-muted">{formatDate(entry.date, locale)}</p></td><td className="px-5 py-4 font-black">{money(entry.amount)}</td><td className="px-5 py-4"><Badge tone={badge.tone}>{badge.label}</Badge></td><td className="px-5 py-4">{canPostRow(entry) ? <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="secondary" disabled={postingId === entry.id || cancelingId === entry.id} onClick={() => postEntry(entry)}>{postingId === entry.id ? common("loading") : t("postAction")}</Button>{canCancelRow(entry) && <Button type="button" size="sm" variant="secondary" disabled={postingId === entry.id || cancelingId === entry.id} onClick={() => cancelDraft(entry)}>{cancelingId === entry.id ? common("loading") : t("cancelAction")}</Button>}</div> : canReverseRow(entry) ? <Button type="button" size="sm" variant="secondary" disabled={reversingId === entry.id} onClick={() => reverseEntry(entry)}>{reversingId === entry.id ? common("loading") : t("reverseAction")}</Button> : <span className="text-muted">—</span>}</td></tr>; })}</tbody></table></div>
            </div>
          ) : (
        ...
        <form onSubmit={submitDraft} className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-3">
            <label>
              ...
              <DateInput required className="input-base" value={draftDate} onChange={setDraftDate} />
            </label>
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-end gap-6">
          <label>
            <span className="label-base">{t("asOf")}</span>
            <DateInput className="input-base" value={asOf} onChange={setAsOf} />
          </label>
          <label className="flex items-center gap-2 pb-2">
        ...
        <Card className="p-5">
          ...
          <form onSubmit={saveLock} className="mt-4 grid gap-3">
            <label>
              ...
              <DateInput className="input-base" value={lockDate} onChange={setLockDate} />
            </label>
            <label>
