...
<div className={`${ux9.surface} space-y-6`}>
      ...
      <Card className="flex flex-wrap items-end gap-3 p-5">
        <label className="space-y-1 text-sm"><span>{ar ? "من" : "From"}</span><DateInput className="block rounded-xl border bg-background px-3 py-2" value={from} onChange={setFrom} /></label>
        <label className="space-y-1 text-sm"><span>{ar ? "إلى / كما في" : "To / as of"}</span><DateInput className="block rounded-xl border bg-background px-3 py-2" value={to} onChange={setTo} /></label>
