...
export default function ExportCenterPage() {
  ...
  // Phase 5B: export the FULL matching dataset (every page) via fetchAllMatching
  // from the proven pagination hooks — not the ~25-row useCoreErpData sample.
  // invoices: posted-only + branch scope (useInvoices); assets: company-wide
  // standalone only (standaloneOnly=true); customers: company scope.
  ...
  const generatedAt = () => formatDateTime(new Date(), "Asia/Dubai", locale);
  ...
  const buildRequest = async (source: ExportSource) => {
    if (source === "sales") {
    ...
    // Tax summary: server-side posted-only aggregate (Phase 5E). Never the
    // truncated useCoreErpData sample.
    const res = await apiClient<any>("/reports/tax-summary", { locale });
    const payload = (res?.data ?? res) ?? {};
    const tt = payload.totals ?? {};
    const num = (v: unknown) => (Number.isFinite(Number(v)) ? Number(v) : 0);
    const rows: ExportRow[] = [
      { metric: salesT("totalSales"), value: num(tt.salesTotal) },
      { metric: exportT("vat"), value: num(tt.vatTotal) },
      { metric: rtl ? "صافي الإجمالي" : "Net subtotal", value: num(tt.netSubtotal) },
      { metric: exportT("records"), value: num(tt.records) },
    ];
    const columns: ExportColumn<ExportRow>[] = [
  ...
  const runExport = async (source: ExportSource, format: ExportFormat) => {
    const request = await buildRequest(source);
  ...
  const handleStartExport = async () => {
    if (isTriggering) return; // prevent double-trigger while fetching all pages
    setIsTriggering(true);
    const now = generatedAt();
    const jobId = `JOB-${Date.now()}`;
    try {
      const result = await runExport(exportSource, exportFormat);
  ...
  const rerunJob = async (job: LocalExportJob) => {
    if (isTriggering) return;
    setIsTriggering(true);
    try {
      const result = await runExport(job.source, job.format);
      if (result.ok) {
        toast.success(exportT("exportReady"));
      } else {
        toast.error(result.errorCode === "empty-data" ? exportT("noDataToExport") : exportT("exportFailed"));
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_1.3fr]">
        <Card className="space-y-5 p-6">
          ...
          <div className="space-y-4">
            ...
            {exportSource === "tax" && (
              <p className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-navy-950 dark:text-slate-300">
                {rtl
                  ? "ملخص ضريبي مبني على الفواتير المرحلة (server-side، posted فقط)."
                  : "Tax summary computed server-side from posted invoices only."}
              </p>
            )}
              ...
              <div key={job.id} className="space-y-3 rounded-2xl border border-slate-100 p-4 dark:border-slate-800">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <span className="mt-1 block text-[10px] text-slate-400">{formatDateTime(job.createdAt, "Asia/Dubai", locale)} · ID: {toEnglishDigits(job.id)}</span>
                  </div>
                ...
                <div className="flex justify-end pt-1">
                  <Button variant="secondary" size="sm" className="h-8 py-1 text-xs" disabled={isTriggering} onClick={() => rerunJob(job)}>
