...
try {
  ...
  check(roleService.includes("reservationAdvancesAccountId") && roleService.includes("adopted"), "existing valid setting is adopted without replacement");
  check(roleService.includes("created: []") && roleService.includes("alreadyPresent"), "company bootstrap reports idempotent outcomes");
  check(!/CMP-DEMO|Company\.findOne/i.test(roleService), "bootstrap has no demo or first-company fallback");
  check(reservation.includes("resolveSystemAccountRole(companyId, SYSTEM_ACCOUNT_ROLES.CUSTOMER_DEPOSIT_LIABILITY"), "reservation cannot choose liability account from client input");
  ...
  check(routes.includes("/readiness/operations") && routes.includes("/bootstrap/company-accounts"), "bounded company readiness and bootstrap endpoints exist");
