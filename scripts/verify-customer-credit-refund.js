...
function verifyBackendRoute() {
  ...
  assertNotMatches(refundSection, /["'](?:1110|1120)["']/, "payload normalization has no fixed treasury account authority");
  ...
  assertIncludes(route, "debitAccountId: depositAccount.id", "GL debit uses the resolved branch deposit account");
  assertIncludes(route, "creditAccountId: treasuryAccount.id", "GL credit uses the authoritative mapped treasury account");
...
function verifyFrontend() {
  ...
  const submitRefund = sliceBetween(page, "const submitRefund", "const submitApplyCredit");
  ...
  assertIncludes(submitRefund, "amount,", "refund payload retains amount");
  assertIncludes(submitRefund, "paymentMethod: refundForm.paymentMethod", "refund payload retains payment method");
  assertIncludes(submitRefund, "date: refundForm.date", "refund payload retains date");
  assertIncludes(submitRefund, "description: refundForm.description.trim()", "refund payload retains description");
  assertIncludes(submitRefund, "reference: refundForm.reference.trim()", "refund payload retains reference");
  assertNotMatches(submitRefund, /accountCode|accountId|depositAccount|liabilityAccount|treasuryAccount|branchId|companyId/, "normal refund payload cannot select an account or authority scope");
  assertIncludes(submitRefund, "setRefundSubmitting(true)", "refund blocks duplicate submission while pending");
  assertIncludes(submitRefund, "toast.error(err?.message", "refund shows a safe server error without false success");
