...
function staticContract() {
  ...
  contains(moduleAccess, "resolveEmployeeWorkspaceRoute", "one canonical post-verification route resolver exists");
  contains(verificationShell, "resolveEmployeeWorkspaceRoute(authorization)", "inline verification routes using the fresh server authorization");
  contains(operatorBar, "resolveEmployeeWorkspaceRoute(authorization)", "Change Employee uses the same fresh-authorization route resolver");
  contains(authGuard, "resolveEmployeeWorkspaceRoute(operator.authorization)", "reload recovery uses the canonical Employee workspace resolver");
  contains(authGuard, "noAssignedBusinessAccess", "Employees without business permissions receive a controlled no-access state");
  contains(authGuard, "data-no-assigned-employee-access", "the no-access state is explicit and testable");
  assert.ok(!verificationShell.includes('router.replace("/pos")'), "inline verification has no POS fallback");
  assert.ok(!operatorBar.includes("pendingEmployeeId"), "Change Employee does not route from an asynchronously stale provider snapshot");
  ...
  contains(operatorBar, "handleChangedEmployee", "Change Employee receives the fresh replacement authorization before routing");
  ...
  contains(read("app/providers.tsx"), "isTerminalTechnicalAuthError(error)", "query handling sends only terminal technical errors to the shared coordinator");
